...
internal fun IssueSummaryDto.mergeIssuePatch(previous: IssueSummaryDto, nowMs: Long): IssueSummaryDto {
    val previousItems = previous.items.associateBy { it.source to it.id }
    return copy(items = items.map { item ->
        val old = previousItems[item.source to item.id]
        if (old == null || item.isFreshForIssuePatch(nowMs)) {
            item
        } else {
            val merged = item.copy(
                url = item.url ?: old.url,
                description = item.description.ifBlank { old.description },
                notes = item.notes.ifBlank { old.notes },
            )
